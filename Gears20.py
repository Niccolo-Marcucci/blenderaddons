...
from math import pi as PI, pow, sin, cos, tan, asin, acos, atan2, degrees, radians, sqrt
import bpy
import bmesh
...
def updateObjects(context):
    ...
    for g in gears:
        ...
        spinangle = 2*asin(g.width*tan(g.helicalangle)/2/radius)
        nsteps = int(abs(spinangle) / radians(8.11))+1      #8.11 allows an error on the radius < 1%
        ...
        for i in range(nsteps):  # we could spin all in one go but for the necessary tapering
            spinangle = 2*asin(g.width/nsteps*tan(g.helicalangle)/2/radius)
            ret = bmesh.ops.spin(
                    bm,
                    geom=geom,
                    angle=spinangle,
